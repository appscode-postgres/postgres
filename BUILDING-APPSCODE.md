...
```
PostgreSQL 16.15 - Postgres Enterprise by AppsCode on x86_64-pc-linux-gnu, ...
```
...
`server_version_num` (`160015`) and the `PG_VERSION` data-directory file
...
Every surface must begin with `PostgreSQL 16.15` and carry the brand as a
suffix:

```sh
bin/postgres --version      # postgres (PostgreSQL) 16.15 - Postgres Enterprise by AppsCode
bin/pg_config --version     # PostgreSQL 16.15 - Postgres Enterprise by AppsCode
bin/initdb -D data -U postgres
bin/pg_ctl -D data -l server.log -w start
grep starting server.log    # LOG:  starting PostgreSQL 16.15 - Postgres Enterprise by AppsCode on ...
bin/psql -U postgres -Atc 'SELECT version();'
                            # PostgreSQL 16.15 - Postgres Enterprise by AppsCode on ..., compiled by ...
bin/psql -U postgres -Atc 'SHOW server_version;'      # 16.15 - Postgres Enterprise by AppsCode
...
`Postgres Enterprise by AppsCode 16.15 on ...`. That broke third-party
...
- `server_version` now carries the brand suffix (`16.15 - Postgres
  Enterprise by AppsCode`), exactly as it does on Percona and Debian
  builds. Anything that must have a bare number should read
  `server_version_num` (`160015`), which is unchanged; that is what libpq
