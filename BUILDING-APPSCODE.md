...
This branch (`AC_16_10`) is upstream PostgreSQL 16.10 (`REL_16_10`) plus a
...
```
PostgreSQL 16.10 - Postgres Enterprise by AppsCode on x86_64-pc-linux-gnu, ...
```
...
`server_version_num` (`160010`) and the `PG_VERSION` data-directory file
...
- Upstream tag: `REL_<MAJOR>_<MINOR>` (this branch: `REL_16_10`)
- AppsCode branch: `AC_<MAJOR>_<MINOR>` (this branch: `AC_16_10`)
...
## New point release (e.g. 16.11)

```sh
git fetch --tags origin
git checkout -b AC_16_11 REL_16_11
git cherry-pick <rebrand commit from AC_16_10>
```
...
Every surface must begin with `PostgreSQL 16.10` and carry the brand as a
suffix:

```sh
bin/postgres --version      # postgres (PostgreSQL) 16.10 - Postgres Enterprise by AppsCode
bin/pg_config --version     # PostgreSQL 16.10 - Postgres Enterprise by AppsCode
bin/initdb -D data -U postgres
bin/pg_ctl -D data -l server.log -w start
grep starting server.log    # LOG:  starting PostgreSQL 16.10 - Postgres Enterprise by AppsCode on ...
bin/psql -U postgres -Atc 'SELECT version();'
                            # PostgreSQL 16.10 - Postgres Enterprise by AppsCode on ..., compiled by ...
bin/psql -U postgres -Atc 'SHOW server_version;'      # 16.10 - Postgres Enterprise by AppsCode
bin/psql -U postgres -Atc 'SHOW server_version_num;'  # 160010 (unchanged)
```
...
`Postgres Enterprise by AppsCode 16.10 on ...`. That broke third-party
...
- `server_version` now carries the brand suffix (`16.10 - Postgres
  Enterprise by AppsCode`), exactly as it does on Percona and Debian
  builds. Anything that must have a bare number should read
  `server_version_num` (`160010`), which is unchanged; that is what libpq
