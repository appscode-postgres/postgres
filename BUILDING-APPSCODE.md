...
byte-for-byte identical to upstream (`server_version` = `16.10`,
`server_version_num` = `160010`, `PG_VERSION` data-directory file = `16`),
...
All of these must print exactly `Postgres Enterprise by AppsCode 16.10`
as the product+version (no suffix on the number):

```sh
bin/postgres --version      # postgres (Postgres Enterprise by AppsCode) 16.10
bin/pg_config --version     # Postgres Enterprise by AppsCode 16.10
bin/initdb -D data -U postgres
bin/pg_ctl -D data -l server.log -w start
grep starting server.log    # LOG:  starting Postgres Enterprise by AppsCode 16.10 on ...
bin/psql -U postgres -Atc 'SELECT version();'
                            # Postgres Enterprise by AppsCode 16.10 on ..., compiled by ...
bin/psql -U postgres -Atc 'SHOW server_version;'      # 16.10  (unchanged)
