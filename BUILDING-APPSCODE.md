...
```
PostgreSQL 17.11 - Postgres Enterprise by AppsCode on x86_64-pc-linux-gnu, ...
```
...
`server_version_num` (`170011`) and the `PG_VERSION` data-directory file
(`17`) are byte-for-byte identical to upstream.
...
Every surface must begin with `PostgreSQL 17.11` and carry the brand as a
suffix:

```sh
bin/postgres --version      # postgres (PostgreSQL) 17.11 - Postgres Enterprise by AppsCode
bin/pg_config --version     # PostgreSQL 17.11 - Postgres Enterprise by AppsCode
bin/initdb -D data -U postgres
bin/pg_ctl -D data -l server.log -w start
grep starting server.log    # LOG:  starting PostgreSQL 17.11 - Postgres Enterprise by AppsCode on ...
bin/psql -U postgres -Atc 'SELECT version();'
                            # PostgreSQL 17.11 - Postgres Enterprise by AppsCode on ..., compiled by ...
bin/psql -U postgres -Atc 'SHOW server_version;'      # 17.11 - Postgres Enterprise by AppsCode
...
`Postgres Enterprise by AppsCode 17.11 on ...`. That broke third-party
...
- `server_version` now carries the brand suffix (`17.11 - Postgres
  Enterprise by AppsCode`), exactly as it does on Percona and Debian
  builds. Anything that must have a bare number should read
  `server_version_num` (`170011`), which is unchanged; that is what libpq
